...
from pycolor import wallpaper, utils
...
def get_random_image(img_dir, recursive):
    """Pick a random image file form a directory."""
    if recursive:
...
def get_next_image(img_dir, recursive):
    ...
    images.sort(key=lambda img: [int(x) if x.isdigit() else x for x in re.split('([0-9]+)', img)])
...
def get(img, cache_dir=CACHE_DIR, iterative=False, recursive=False):
    ...
    wall_img = os.path.abspath(wal_img)

    # Cache the image file path.
    utils.save_file(wall_img, os.path.join(cache_dir, "wal"))
